...
> **⚠️ PARTIALLY STALE.** Provider/latency/cost details and the original module layout (since consolidated 5 → 3 packages) are outdated. The inference provider changed several times since; the live image path is now fal.ai's hosted FLUX.2-klein realtime, with a Lambda Cloud dev path (the RunPod system, including Redis, was removed 2026-07-17) — see `CLAUDE.md` and `documents/decisions.md`. **The data model + business model here are also superseded:** user accounts + a monthly fal-spend ledger live in **Postgres** today (`users` + `monthly_usage`, see `backend/src/postgres/schema.sql`), not the `users`/`usage_log`/tier schema sketched below; billing is a $10/month fal-spend cap (subscription purchase deferred). Treat as historical context for original design intent. For current architecture, read the code and `CLAUDE.md`.

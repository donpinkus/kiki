...
# ─── LTX-2.3 (video server — dedicated Lambda H100) ─────────────────────────
# Restored 2026-07-18 for the Lambda video port (was removed with the RunPod
# path 2026-07-17; original extract preserved in archive/video-ltx/).
# The video server runs LTX-2.3 22B distilled FP8 via Lightricks' official
...
# accepted the license. Server runtime stays offline.
...
# end-to-end test.
#
# 512x512 squares the aspect (matches FLUX 1:1 output, no pillarboxing in
# the iPad's square pane) and keeps activation memory low: 512 = 8x64
# satisfies the DistilledPipeline two-stage divisor. Upscale to display
# size happens iPad-side via AVPlayerLayer's resizeAspect.

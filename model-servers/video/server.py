...
Auth: `/ws` requires `?token=<KIKI_WS_TOKEN>` when the env var is set (Lambda
instances have a public IP with 8766 open account-wide — same gate as
image/server.py). TLS: serves wss when LTX_SSL_CERT/LTX_SSL_KEY point at the
fleet's shared self-signed cert (backend pins it via LAMBDA_TLS_CA_B64).
...
serializes via its 3s-idle trigger, but new sketches can interleave).
...
async def websocket_video(ws: WebSocket):
    ...
    # Shared-secret gate: the Lambda instance sits on a public IP with the
    # port open to the internet, so /ws requires the per-instance token
    # (HMAC of the instance name, minted by the backend / launch script).
    # No-op when KIKI_WS_TOKEN is unset (local dev unchanged). Same pattern
    # as image/server.py.
...
if __name__ == "__main__":
    # TLS: when LTX_SSL_CERT/KEY are set (Lambda fleet — the shared
    # self-signed cert on the filesystem that the backend PINS via
    # LAMBDA_TLS_CA_B64), serve wss/https. Unset = plain ws (local dev).
    _ssl_cert = os.environ.get("LTX_SSL_CERT") or None
    _ssl_key = os.environ.get("LTX_SSL_KEY") or None
    uvicorn.run(
        "video.server:app",
        host=config.HOST,
        port=config.PORT,

...
async def run_test(args):
    ...
    if args.url.startswith("wss") and args.insecure:
        # Lambda fleet serves a self-signed cert (backend pins it; this dev
        # client just skips verification).
        import ssl
        ssl_ctx = ssl.create_default_context()
        ssl_ctx.check_hostname = False
        ssl_ctx.verify_mode = ssl.CERT_NONE

    async with websockets.connect(args.url, max_size=50 * 1024 * 1024, ssl=ssl_ctx) as ws:
        # Initial status
        msg = await ws.recv()
...
def main():
    ...
    parser.add_argument("--insecure", action="store_true",
                        help="Skip TLS verification (Lambda fleet's self-signed cert)")

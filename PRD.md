...
> **⚠️ PARTIALLY STALE.** Provider references, latency numbers, and cost models are outdated — the inference provider changed several times since this was written, and the live image path is now our own FLUX.2-klein servers on a Lambda Cloud H100 pool with fal.ai's hosted FLUX.2-klein realtime as fallback (`IMAGE_PROVIDER=auto`; the RunPod system was removed 2026-07-17, and video idle-state animation is archived in `archive/video-ltx/`). See `CLAUDE.md` and `documents/decisions.md`. **The pricing/business model (the $9.99 Plus / $19.99 Pro tiers, daily generation limits) is also superseded:** the current model is a per-user **$10/month drawing-spend cap** for unsubscribed users (test accounts exempt), and the Apple subscription purchase flow (StoreKit 2, ~$4/mo) is built — only the App Store Connect product setup remains. Product vision, UX spec, and App Store requirements remain valid. For current implementation, read the code and `CLAUDE.md`.

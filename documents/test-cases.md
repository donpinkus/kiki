...
55. **Proportions.** The Proportions chips (Default, Chibi, Fashion, Heavy, Slim) reshape the figure live without changing its pose or position; Adjust reveals six sliders (Height, Head, Torso, Limbs, Hands & feet, Build) that move continuously, and a chip only stays highlighted while the sliders match it exactly. Limbs bend cleanly on a Chibi (no shearing at elbows/knees). Place, then Edit Pose: the figure reopens with the same proportions. Lying and swimming poses open side-on. Switching body keeps the pose. Place bakes and restores the drawing chrome; Cancel restores it untouched.

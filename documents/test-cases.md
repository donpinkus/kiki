...
54. **Pose panel.** The left panel's pose grid (Standing, Moving, Jumping, Low, Sitting, Action) scrolls; tapping a pose snaps the figure into it without moving it or changing the body, rings the tile, and the joints remain draggable afterwards (dragging one un-rings the tile). The View row turns the figure to Front / ¾ / Side / Back and highlights the current view; free orbiting clears the highlight. Mirror flips the pose left↔right (and the ¾ view to the other side); tapping it twice restores the pose exactly.

55. **Proportions.** The Proportions chips (Adult, Teen, Child, Toddler, Chibi, Fashion, Heavy, Slim) reshape the figure live without changing its pose or position; Adjust reveals six sliders (Height, Head, Torso, Limbs, Hands & feet, Build) that move continuously, and a chip only stays highlighted while the sliders match it exactly. Limbs bend cleanly on a Child or Chibi (no shearing at elbows/knees). Place, then Edit Pose: the figure reopens with the same proportions. Lying and swimming poses open side-on. Switching body keeps the pose. Place bakes and restores the drawing chrome; Cancel restores it untouched.

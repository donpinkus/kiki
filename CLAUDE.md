...
**Video idle-state animation (Lambda port POC, 2026-07-18):** LTX-2.3 image-to-video on a **dedicated** Lambda H100 (`kiki-video-*` instances + `kiki-video-<region>` filesystem — never shared with the image pool: LTX 22B FP8 + Gemma ≈ 46 GiB resident can't fit beside the 9B-KV image server on one 80 GB card, which also guarantees image latency is never impacted). Trigger: the backend fires ONE `video_request` per idle period, **3 s after the user's last sketch frame** (`VIDEO_IDLE_TRIGGER_MS`), animating the newest **generated** image (waits for the final in-flight generation if needed); drawing again cancels the in-flight video. Provider-agnostic (works over fal or lambda image paths — the old pod-side `queueEmpty` trigger is retired, though the fal relay still synthesizes the meta). Gate: `LAMBDA_VIDEO_POOL_ENABLED=true` (production — a **managed video pool** sharing the image pool's orchestration via `modules/lambda/instancePool.ts`, the factored-out `devPool` machinery: interest-launched on app/stream open, least-loaded assignment, 3-strike health kills, 30-min idle reap, redeploy adoption; **deliberately slow scaling** — `LAMBDA_VIDEO_POOL_TARGET_STREAMS=8`, `MAX=1` by default, since videos are one-shot ~15-30s serialized jobs and overload just means late/cancelled videos) or a static `LAMBDA_VIDEO_URL` dev override (takes precedence). Sessions that start before the pool warms are image-only and lazily upgrade mid-session. Best-effort throughout — any video failure degrades to image-only, never an iPad-visible error; "no video" is the fallback (no fal equivalent). `lambda_pool_events` now carries a `pool` column ('image'/'video'). Code: `model-servers/video/` (server restored from `archive/video-ltx/` + token gate + wss), `backend/src/modules/video/videoSession.ts` (idle trigger + relay, mock-server tested), hooks in `routes/stream.ts`, scripts `backend/scripts/lambda/{setup-lambda-video,launch-video}.ts`. iOS's video render path (`VideoEvent` parsing, `ResultState.videoStreaming/.videoLooping`, `LoopingVideoView`) was never removed; the only iOS additions are the top-bar **Animate button** (`DrawingTopBar` → `{type:'animate'}` up the stream WS → backend fires immediately, bypassing the idle window) and its state (`AppCoordinator.isAnimating`, driven by the backend's `video_started` on every fire — auto or manual — and reset by `video_complete_data`/`video_cancelled`, incl. synthesized can't-animate replies). Not yet live-tested on an H100 (needs `LAMBDA_API_KEY`+`HF_TOKEN`); no pool/autoscaler and no metering yet. Full runbook + gaps: `documents/plans/lambda-video-provider.md`. License gotcha: LTX-2 Community License Agreement (NOT Apache-2.0, restricts commercial use ≥$10M revenue) — verify before App Store submission.
...
- **Drawing view** (`DrawingView`) — canvas + result pane. Gallery button top-left navigates back. Stream starts automatically when entering a drawing. Two layouts (Settings → Display, `AppCoordinator.drawingLayout`): **split-screen** (fixed result pane on the left half) and **fullscreen** (the canvas drawing surface fills the available pane below the toolbar; the result floats as the panel below).

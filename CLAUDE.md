...
- **Speed-paint replay page (2026-08-23, replaces the replay modal)** — Share menu → "Speed paint replay" opens `AppScreen.replay` (`Views/SpeedPaintReplayView.swift`), a full screen: near-fullscreen looping preview of the stitched canvas+generated composition (no encode pass — layout/speed re-compose live; MP4 encodes eagerly in the background for instant share taps), layout (side-by-side/stacked) + speed (12s default · 1x · 2x · 5x · 10x) pickers + watermark toggle in one row, three horizontal share buttons (Instagram / Photos / Share…). Entry checkpoints the live recorder BEFORE `stopStream()` (so just-drawn strokes are included without racing the detached finalize); Back re-queues canvas pending state + restarts the stream (same pattern as Animate). RootView deliberately skips the cross-fade into this screen (video layer born inside an animated-opacity group is a gray-preview suspect). Every player attach runs the `VideoDiag` watchdog (see Observability → Gray-video-preview diagnostics). Sim dev actions: `replayModal`/`replayScreen` (open), `dismiss` (back).
...
- **Extract screen (2026-07-20, reworked 2026-08-23)** — "Extract" button next to Animate (needs a generated image; extraction acts on the GENERATED image, never the sketch — owner decision). Full screen (`ExtractView` + `App/ExtractController`; the drawing stream is STOPPED while open, restarted on close — same pattern as Animate). Image left with **multi-point SAM refinement**: Add/Remove taps jointly re-decode the selection over all accumulated points (`SAM2Segmenter.bundled()` re-encoded per open; live contour preview via `MaskContour.normalizedLoops`, mask post-processing off-main; undo/Clear; sim Core ML zeroes decoder masks → `extractFake:u,v,r` dev action injects a circle and lifts it immediately), then an explicit **"Lift to 3D"** confirm cuts out (CIBlendWithMask, 1px feather, bbox-cropped) and starts a Hunyuan lift (`/v1/lift3d`). Upload sizing lives in `App/Lift3DUpload.swift` (white composite, 256px-short-side upscale + pad-to-128 floor — fal rejects any side <128px, "Image resolution only support [128, 5000]"; shared with the object drawer's lift so the copies can't diverge). Lift cards show a determinate ETA from measured durations (UserDefaults median, seeded 90s; real lifts ~80–90s); "Save to Collection" persists a `SavedObject` (cutout + GLB + mesh thumb). **Closing mid-lift is safe**: finished-unsaved lifts auto-save on close, in-flight ones keep running (`prepareForBackground` sheds SAM models/source image/finished blobs — jetsam matters here), auto-save on completion with a toast + count badge on the toolbar shippingbox (badge clears when the drawer appears). Telemetry: iOS `objects.lift_started/completed/failed` + backend `objects.lift3d` → Insights; `extract.*` Sentry logs double as breadcrumbs carrying memory footprint (watchdog forensics). Sim dev actions: `extractScreen` / `extractFake:` / `extractSave` / `dismiss` (closes the screen).
...
- **Document resolution (fixed, decoupled from view size)**: layer/scratch textures are allocated **once** at a fixed `2048²` (`MetalCanvasView.documentSide`) by `CanvasRenderer.configureDocument` — they are **NOT** sized to the view. `layoutSubviews` updates only `CAMetalLayer.drawableSize` (which tracks the view); the compositor scales the fixed document onto the drawable. So a view/layout change (style-picker `fullScreenCover`, rotation) only restrides the *display* — the document is **never reallocated or resampled**, so resolution is preserved and strokes are never wiped on resize. Touch→texture mapping funnels through `canvasScale` = document÷view ratio (refreshed every `layoutSubviews`), so on-screen brush size is view-size-invariant. **Do not** re-tie the texture size to `bounds` — that's the exact regression fixed on 2026-06-06 (the style picker's transient bounds wobble was wiping/blurring the canvas; see commit `4f99ea7`).
- **Brush rendering**: instanced stamp quads. Touch points → arc-length resampled positions → `StampInstance` buffer → single instanced draw call per frame. Adaptive spacing (stamp gap = 30% of pressure-modulated width) keeps strokes dense at all pressures. **Stroke walks are stateful + incremental (2026-09-10):** `DryStrokeWalker` advances over only the points added per touch batch (the whole-stroke re-walk per `touchesMoved` was O(n²)); `StrokeStampGenerator.stamps(for:)` = init+advance+finish on the same walker, so finalization is byte-identical to the preview (offline-asserted, `OfflineTests/walkers`). `EraserStrokeWalker` / `WetStrokeWalker` are the same pattern. Walk-length constants (spacing floors, sag cap, Fall Off, Charge, Speed/Distance sensor periods) are DOCUMENT-PIXEL values divided by the walk scale (`StrokeWalkUnits`), so the harness at scale 1 matches the iPad and every iPad size matches the 12.9" reference. The selection clip is baked once into a `StampClip` bitmap (one array read per dab).
- **Eraser**: `EraserStrokeWalker` stamps applied directly to canvas texture with destination-out blend, per touchesMoved (first dab at touch-down). Undo snapshot taken at touchesBegan.
...
- **Undo**: full-texture CPU snapshots (`texture.getBytes()` → `LayerSnapshotBlob`, LZ4-compressed off-main), depth 30 AND a 256 MB resident budget, evicted on memory warnings. Every entry records the stroke count before its mutation so undo/redo restore it exactly. Restore via `texture.replace()` behind a fence that drains any in-flight eraser/wet batch.
...
**Gray-video-preview diagnostics (iOS `VideoDiag`, `App/VideoPlaybackDiagnostics.swift`, added 2026-08-22 — keep for several weeks).** Hunting a device-state bug where AVPlayerLayers render gray app-wide until the iPad reboots (observed on iPadOS 26 hardware; same binary played fine post-reboot). Every player attach (replay page/modal, Animate clips) runs a watchdog: healthy → `event:video.playback_ok` (Logs, with time-to-display); gray → `event:video.playback_no_display` (Logs ERROR + Errors UI, tagged `video_verdict`) carrying the discriminating evidence: `frames_vended` (decoded-but-not-composited vs not-decoding), an independent AVAssetWriter/Reader codec probe (`probe_encode_ok`/`probe_decode_ok` — daemon/VT exhaustion vs playback-pipeline wedge), memory/thermal, `device_uptime_s` (reboot correlator), and cross-launch episode counters. `event:video.media_services_lost/_reset` logs media-daemon deaths. When the gray state recurs, query `event:video.playback_no_display` and read the `verdict`.

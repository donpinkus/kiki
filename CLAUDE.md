...
- **Magic wand (SAM tap-to-select)**: on-device SAM 2.1-small Core ML (apple/coreml-sam2.1-small, Apache 2.0, ~90 MB committed under `CanvasModule/Resources/SAM2/`, re-fetch via `ios/scripts/fetch-sam-models.sh`). Tap = point prompt (Add/Subtract toggle → positive/negative), "New Object" accumulates disjoint objects, union mask → marching-squares CGPath → the SAME `setClipPath` clip+ants machinery as lasso (clip `contains` switched to `.evenOdd` for holes/disjoint subpaths). Encoder runs once per image (~2-5 s, cached until canvas changes between objects); per-tap decode is fast. Source image = generated image in overlay layout, else flattened canvas. Code: `CanvasModule/MagicWand/` (SAM2Segmenter actor, MaskContour, MagicWandController) + wand UI in `DrawingTopBar`/`DrawingView`. **Gotcha: the iOS SIMULATOR's Core ML returns all-zero mask logits for SAM's decoder (scores fine; broken on 18.3.1 + 26.5, CPU and GPU) while the same .mlmodelc works on macOS + device — sim tests use the `wandFake:` dev action (synthetic mask) and the macOS harness pattern in scratchpad; device verification via `-KikiWandSelfTest 1` launch arg + `ipad.sh logs`.**
